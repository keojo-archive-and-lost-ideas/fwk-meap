...
describe('createShadowFragment', () => {
    test('it create a virtual node', () => {
        const node = createShadowFragment({ vNodes: ['foo', 'bar'] });
        expect(node).toEqual({
            type: 'fragment',
    ...
    test('create virtual node with nested children', () => {
        const node = createShadowFragment({ vNodes: ['foo', ['bar', 'baz']] });
        expect(node).toEqual({
            type: 'fragment',
...
describe('createShadowElement', () => {
    test('creates a virtual node', () => {
        const node = createShadowElement('div', { id: 'foo' }, ['bar']);
        expect(node).toEqual({
            tag: 'div',
            props: { id: 'foo' },
        ...
        test('remove nulls from children', () => {
            const node = createShadowElement('div', { id: 'foo' }, ['bar', null, 'baz']);
            expect(node).toEqual({
                tag: 'div',
                props: { id: 'foo' },
                children: [{ type: 'text', value: 'bar' }, { type: 'text', value: 'baz' }],
                type: 'element',
            });
        }),
        test('handles nested children', () => {
            const node = createShadowElement('div', { id: 'foo' }, ['bar', null, ['baz', null, 'qux']]);
            expect(node).toEqual({
                tag: 'div',
                props: { id: 'foo' },
                children: [{ type: 'text', value: 'bar' }, ['baz', null, 'qux']],
                type: 'element',
            });
        });
