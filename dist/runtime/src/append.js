...
export function appendTextNode({ node, parentElement }) {
    const { value } = node;
...
export function appendFragmentNode({ node, parentElement }) {
...
export function appendElementNode({ node, parentElement }) {
...
function addPropsToElement({ element, props, vNode }) {
    if (props === null)
        return;
    const { on: events } = props, attributes = __rest(props, ["on"]);
